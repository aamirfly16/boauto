...
from Autobo.Auth.models import Auths, Rmtest11, Rrpage13, Pmtest3, easset4, Bopage4, pcrenew12, Rmfinal1, Pmfinal, eassetfinal1, Rmlive4, Rmtest11
...
# activate_this = "Z:\app.05\<--virtual env name -->\Scripts\\activate_this.py" #for windows
# activate_this = "C:\Users\ppasha\PycharmProjects\GESBO.01\envs\my_test3\Scripts\\activate_this.py" #for windows
# with open(activate_this) as f:
# code = compile(f.read(), activate_this, 'exec')
# exec(code, dict(__file__=activate_this))

# activate_this_file = "C:\Users\ppasha\PycharmProjects\GESBO.01\anaconda3\envs"
# exec(compile(open(activate_this_file, "rb").read(), activate_this_file, 'exec'), dict(my_test2=activate_this_file))
...
# def create_app(config_type):
    ...
    # return app
# dbname='{your_database}' user='postgresbo@gesbo' host='gesbo.postgres.database.azure.com' password='{your_password}' port='5432' sslmode='true'
...
main.config.update(
    ...
    SQLALCHEMY_DATABASE_URI = 'postgresql://postgresbo@gesbo:Lkjfdsa123@gesbo.postgres.database.azure.com:5432/auto_bo',
...
if __name__ == '__main__':
    # host = "<gesbo.postgres.database.azure.com>"
    # dbname = "<auto_bo>"
    # user = "<gesbo>"
    # password = "<Lkjfdsa123>"
    # sslmode = "require"
    ...
    engine = create_engine("postgresql+psycopg2://postgresbo@gesbo:Lkjfdsa123@gesbo.postgres.database.azure.com:5432/auto_bo")
    conn = psycopg2.connect(dbname='auto_bo', user='postgresbo', password='Lkjfdsa123', host='gesbo.postgres.database.azure.com', port='5432', sslmode='require' )
...
def results():
    ...
    rrpage13=Rrpage13
    # Pcrenew12=pcrenew12
    engine5 = create_engine("postgresql+psycopg2://postgresbo@gesbo:Aaasss@123@gesbo.postgres.database.azure.com:5432/auto_bo")
...
def dftestmerge():
    ...
    engine1 = create_engine("postgresql+psycopg2://postgresbo@gesbo:Aaasss@123@gesbo.postgres.database.azure.com:5432/auto_bo")
    engine2 = create_engine("postgresql+psycopg2://postgresbo@gesbo:Aaasss@123@gesbo.postgres.database.azure.com:5432/auto_bo")
...
def edit_rm(id):
    ...
    cur.execute('SELECT * FROM Rmtest11 WHERE id = {0}'.format(id))
    data = cur.fetchall()
    cur.close()
    print(data[0])
    return render_template('rmedit.html', Rmtest1=data[0])
...
def get_edit(id):
    ...
    cur.execute('SELECT * FROM Rrpage13 WHERE id = {0}' .format(id))
    data = cur.fetchall()
    cur.close()
    print(data[0])
    return render_template('edit.html', Rrpage1=data[0])
...
def update_bo(id):
    if request.method == 'POST':
        ...
        cur.execute("""
            UPDATE Rrpage13
            SET it_request_number = %s,
                unshare_it_req_number = %s,
                it_connect_stage = %s,
                bo_status = %s,
                remarks = %s
            WHERE id = %s
...
def importxl():
    ...
    engine3 = create_engine("postgresql+psycopg2://postgresbo@gesbo:Aaasss@123@gesbo.postgres.database.azure.com:5432/auto_bo")

    with pd.ExcelFile('xlimport\Book1.xlsx') as xls:
...
def importxlpm():
    ...
    with pd.ExcelFile('xlimport\Projectmaster.xlsx') as xls:
        dfpmxl = pd.read_excel(xls)
        dfpmxl.to_sql(name='pmtest3', con=engine, if_exists='replace', index=False)
...
def importxlpcrenew():
    ...
    with pd.ExcelFile('xlimport\PCRenewalReport11.xlsx') as xls:
        dfpcxl = pd.read_excel(xls)
        dfpcxl.to_sql(name='pcrenew12', con=engine, if_exists='replace', index=False)
    engine.dispose()
...
def importrmlive1xl():
    ...
    with pd.ExcelFile('xlimport\live2_rm.xlsx') as xls:
...
def importeassetxl():
    ...
    with pd.ExcelFile('xlimport\Easset.xlsx') as xls:
